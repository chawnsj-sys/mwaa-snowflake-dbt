...
- GitHub Secrets 中配置 `AWS_ROLE_TO_ASSUME`（GitHub OIDC 使用的 IAM Role ARN）
- GitHub Variables 中配置 `MWAA_S3_BUCKET`（MWAA DAG S3 桶名）
...
| 用户 | 角色 | 用途 | 状态 |
|------|------|------|------|
| `<YOUR_SNOWFLAKE_USER>` | ACCOUNTADMIN / SYSADMIN | 管理员，环境初始化、dbt 执行 | ✅ 活跃 |
...
| 配置项 | 值 |
|--------|-----|
| S3 桶 | `s3://snowflake-ingestion-<YOUR_AWS_ACCOUNT_ID>/` |
...
```
S3 桶结构：
  s3://snowflake-ingestion-<YOUR_AWS_ACCOUNT_ID>/
...
**当前状态：⚠️ Snowpipe 已配置但 DAG ingest 步骤暂停**

| 组件 | 状态 | 说明 |
|------|------|------|
| S3 桶 | ✅ 已创建 | `s3://snowflake-ingestion-<YOUR_AWS_ACCOUNT_ID>` |
| IAM Role | ❌ 被 GuardDuty 阻断 | 信任策略被自动改为 Deny，需手动修复 |
| Storage Integration | ✅ 已创建 | `s3_ingestion_integration` |
| Stage + File Format | ✅ 已创建 | `RAW_LANDING.s3_ingestion_stage` |
| 3 个 Pipe | ✅ 已创建 | `PIPE_CUSTOMERS`、`PIPE_ORDERS`、`PIPE_ORDER_ITEMS` |
| Landing 表 | ✅ 已创建 | `RAW_LANDING.CUSTOMERS/ORDERS/ORDER_ITEMS` |
| DMF 数据质量 | ✅ 已配置 | NULL_COUNT + DUPLICATE_COUNT，数据变更时自动触发 |
| DAG ingest 步骤 | ⏸️ 暂停 | 待 IAM 修复后恢复 |

**恢复步骤：**
```bash
# 1. 修复 IAM Role 信任策略（把 Deny 改回 Allow）
aws iam update-assume-role-policy \
  --role-name snowflake-ingestion-role \
  --policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
      "Principal": {"AWS": "arn:aws:iam::<TRUSTED_PRINCIPAL_AWS_ACCOUNT_ID>:user/<TRUSTED_PRINCIPAL_USER>"},
      "Action": "sts:AssumeRole",
      "Condition": {"StringEquals": {"sts:ExternalId": "ZC10341_SFCRole=6_PeUi0ymEYeBafEWg/JyX6Vfm8MY="}}
    }]
  }'

# 2. 恢复 DAG ingest 步骤（参考 git log 中的 ingest 版本）
```

### 数据质量：Snowflake DMF

ODS 层（RAW_LANDING）配置了 Data Metric Functions，数据变更时自动检测质量问题：

| 表 | 类型 | dbt 策略 | DMF 监控 |
|----|------|----------|----------|
| CUSTOMERS | 维度表 | merge (upsert) | NULL_COUNT(customer_id, email) + DUPLICATE_COUNT(customer_id) |
| ORDERS | 事实表 | append (追加) | NULL_COUNT(order_id, customer_id, total_amount, status) |
| ORDER_ITEMS | 事实表 | append (追加) | NULL_COUNT(order_item_id, order_id, quantity, unit_price) |

- 维度表检查唯一性（DUPLICATE_COUNT），事实表只检查完整性（NULL_COUNT）
- 触发方式：`TRIGGER_ON_CHANGES`（Snowpipe 加载后自动运行）
- 结果查看：`SELECT * FROM SNOWFLAKE.LOCAL.DATA_QUALITY_MONITORING_RESULTS`
- 配置脚本：`scripts/init_dmf.sql`
